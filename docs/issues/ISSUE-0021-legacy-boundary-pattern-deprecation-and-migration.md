...
3. Add migration subtasks under this issue grouped by pattern class (import coupling, stage-order duplication, raw-to-render shortcuts).
4. Remove or isolate legacy paths behind canonical orchestrator boundaries with deterministic regression tests.
5. Update linked documentation (`docs/architecture.md`, `docs/testing.md`, `docs/issues/ISSUE-0013...`) as milestones are completed.
6. Close only when migration inventory is empty and canonical gate evidence is green.
...
- Follow-on residual authority inventory and seam-ranking work is tracked in `docs/issues/ISSUE-0022-residual-runtime-helper-authority-inventory-after-loop-owner-extraction.md`.
