...
| Changed-path skip policy | `scripts/all_green_gate.py` | inventory complete | unknown pending governed-path matrix audit | likely (skip reduction + governed-surface override interactions are high-risk) | TBD | produce explicit governed-path matrix proving frozen-surface edits disable skip reduction and force full governance checks | ☐ |
