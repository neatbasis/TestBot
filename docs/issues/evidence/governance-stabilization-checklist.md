...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Shared governance rules | `scripts/governance_rules.py` | inventory complete | partial (shared-rule intent is documented; concrete helper consolidation path now identified) | yes (shared helper behavior remains duplicated across validators/gate) | **Proposed canonical decision (analysis-only, not yet ratified):** shared governance helpers (including base-ref resolution primitives) should be centralized in `scripts/governance_rules.py` and imported by all governance scripts. | promote the `resolve_base_ref` duplication finding into this row: inventory `resolve_base_ref`/`git_ref_exists` implementations in `validate_issues`, `validate_issue_links`, and `all_green_gate`; define one retained shared helper path and follow-up migration steps | ☐ |
...
- Pass three ownership/call-path inspection confirmed the boundary above, found no manifest parsing/validation logic in `scripts/validate_issues.py`, and identified two remaining reconciliation items: duplicate `resolve_base_ref` implementations and missing producer→consumer round-trip manifest contract test.
