...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Shared governance rules | `scripts/governance_rules.py` | decision made | partial (base-ref helpers are now shared, and `validate_issue_links.py` explicitly fail-closes commit-history checks under fallback unless degraded mode is opted in; remaining tests still need final consolidation around the shared boundary) | partial (`resolve_base_ref`/`git_ref_exists` duplication is reconciled, but consumer-specific fallback semantics are intentionally non-uniform: commit-traceability now enforces stricter fail-closed semantics than diff-oriented consumers) | **Canonical decision (updated):** keep shared git/ref primitives in `scripts/governance_rules.py`, while preserving explicit consumer policy boundaries. Commit-traceability consumers must fail closed on degraded base-ref fallback by default; degraded mode is explicit opt-in only. | align checklist evidence/tests around the tightened commit-traceability semantics and finish boundary-focused deduplication without collapsing consumer-specific policy contracts | ☐ |
