...
| Verification manifest semantics | `scripts/all_green_gate.py`, `scripts/validate_issue_links.py`, `scripts/verification_manifest_contract.py`, `artifacts/verification/*` | verified | yes (single shared manifest contract now defines producer/consumer semantics and deterministic round-trip tests enforce it) | resolved (one canonical interpretation path via shared contract helpers/constants; duplicate required-check meaning removed from validators) | **Canonical contract statement (authoritative):** producer responsibility (`all_green_gate.py`) is to emit canonical manifest shape with required fields (`schema_version`, `run_id`, `generated_at_utc`, `manifest_path`, `required_checks`, `gate`, `summary`, `checks`) and source `required_checks` from `verification_manifest_contract.py`; consumer responsibility (`validate_issue_links.py`) is to validate manifest reference path/run-ID/file/JSON integrity, treat `run_id` + `required_checks` as authoritative semantics, and fail closed when `required_checks` is missing, malformed, or missing canonical entries; `validate_issues.py` must not reinterpret manifest payload semantics. | maintain this shared-contract path and keep round-trip coverage aligned with any manifest schema evolution | ☑ |
