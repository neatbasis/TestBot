...
## Verification evidence manifest references

Issue `Verification` sections may either include explicit command logs or reference a canonical verification manifest generated by `scripts/all_green_gate.py`.

When using a manifest reference, include both:

1. Manifest path in `artifacts/verification/<run-id>.json`.
2. Matching `Run ID: <run-id>` declaration.
...
- Run ID: `20260316T181500Z-1a2b3c4d`
- Gate summary (optional): `artifacts/all-green-gate-summary.json`
```

Governance validation (`scripts/validate_issue_links.py`) enforces that referenced manifests exist and include required canonical gate checks.
