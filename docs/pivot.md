...
1. **Split `sat_chatbot_memory_v2.py` into:**
  - `src/testbot/entrypoints/sat_cli.py` (legacy compatibility wrapper; canonical runtime CLI entrypoint is `testbot.entrypoints.cli`),
...
1. **`ISSUE-0013-A`**: thin-entrypoint migration is **mostly complete** and package-script authority is switched to `src/testbot/entrypoints/cli.py`.
   - Landed status: `_run_canonical_turn_pipeline` delegates into the turn service using receiver-bound stage handlers, and `sat_cli` remains only as a compatibility wrapper pending retirement under ISSUE-0021.
...
- [~] **`ISSUE-0013-A` mostly complete**: package-script authority is switched to `src/testbot/entrypoints/cli.py`; turn-service delegation is landed, with remaining work limited to monolith-authority reduction, compatibility-delegator retirement (`sat_cli`), continued behavior extraction, and dict-contract reduction.
