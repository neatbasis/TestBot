...
- [x] Typed replacement coverage for prior dict blobs/keys:
  ...
  - retrieval posture distinctions are explicit (`empty_evidence` vs `scored_empty`) and covered by deterministic unit tests.
  ...
  - **Residual gap:** policy/validation reasoning payloads and several artifact contracts still carry `dict[str, object]` maps; do not treat Workstream A as complete until those remaining dict-heavy contracts are typed end-to-end.

## Workstream B — Thin orchestration path
- [ ] Replace monolithic turn flow with explicit canonical sequence orchestration (`observe → encode → stabilize → context → intent → retrieve → decide → assemble → validate → render → commit`); **owner module(s):** `src/testbot/application/services/turn_service.py`, `src/testbot/application/services/canonical_turn_runtime.py`, `src/testbot/entrypoints/*`; **required evidence (test/script/doc):** service tests over fake ports + sequence coverage in BDD; **done signal:** no supported runtime path bypasses the canonical stage chain.
...
- [~] Progress (2026-03-19): CLI/satellite startup routing now executes through `src/testbot/entrypoints/sat_cli.py`, and package script authority in `pyproject.toml` now targets `testbot.entrypoints.sat_cli:main`; `sat_chatbot_memory_v2.main(...)` remains as a compatibility delegator.
- [~] Progress (2026-03-19): canonical raw-utterance runtime ownership is now `application/services/canonical_turn_runtime.py::run_canonical_turn_pipeline(...)` delegating to `turn_service.run_canonical_turn_pipeline_service(...)`, reducing direct monolith orchestration authority.
