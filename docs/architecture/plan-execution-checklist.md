...
- [~] DTO status: [x] `CandidateEncodingSet`, [x] `PreRouteState`, [x] `ContextResolvedState`, [x] `EvidenceSet`, [x] `PolicyDecision`, [x] `ValidationResult`, [x] `RenderedResponse`; [~] `TurnObservation`, [~] `IntentResolution`, [~] `AnswerCandidate`, [~] `CommittedTurnState`.
