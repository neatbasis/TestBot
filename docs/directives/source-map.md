...
| Fallback must be exact phrase `I don't know from memory.` when context is insufficient/low-confidence. | `ANSWER_PROMPT` + `context_is_confident` gating + explicit fallback branches in answer assembly. | Deterministic code path sets fallback when retrieval confidence is low, draft is empty, or contract validation fails. | **Enforced**. |
| Factual answers must be grounded by structured provenance (`used_memory_refs` with `doc_id` + `ts`), while normal user output suppresses raw internal markers. | `validate_answer_contract()`, `has_required_memory_citation()`, `build_provenance_metadata()`, `_suppress_internal_citation_markers()`. | Post-generation contract checks use machine-readable provenance references and fallback gating; debug mode can append explicit diagnostics while normal mode strips internal citation markers. | **Enforced**. |
