...
def test_run_canonical_turn_pipeline_service_prefers_explicit_prior_continuity_authority(monkeypatch):
    captured: dict[str, object] = {}

    def _observe(ctx, _runtime):
        ctx.artifacts["turn_observation"] = object()
        return ctx

    def _encode(ctx, _runtime):
        ctx.artifacts["encoded_candidates"] = object()
        return ctx

    def _stabilize(ctx, _runtime):
        ctx.artifacts["stabilized_turn_state"] = {"turn_id": "t-2"}
        return ctx

    def _context(ctx, runtime):
        captured["prior_continuity"] = runtime.prior_continuity
        captured["pending_ingestion_request_id"] = ctx.artifacts["pending_ingestion_request_id"]
        ctx.artifacts["resolved_context"] = {"history_anchors": []}
        ctx.artifacts["hits"] = []
        ctx.artifacts["retrieval_result"] = {"posture": "empty_evidence", "retrieval_candidates_considered": 0, "hit_count": 0}
        return ctx

    monkeypatch.setattr(turn_service, "observe_turn_stage", _observe)
    monkeypatch.setattr(turn_service, "encode_candidates_stage", _encode)
    monkeypatch.setattr(turn_service, "stabilize_pre_route_stage", _stabilize)
    monkeypatch.setattr(turn_service, "context_resolve_stage", _context)
    monkeypatch.setattr(turn_service, "intent_resolve_stage", lambda ctx, _runtime: ctx)
    monkeypatch.setattr(turn_service, "retrieve_evidence_stage", lambda ctx, _runtime: ctx)
    monkeypatch.setattr(turn_service, "policy_decide_stage", lambda ctx, _runtime: ctx)
    monkeypatch.setattr(
        turn_service,
        "answer_assemble_stage",
        lambda ctx, _runtime: (ctx.artifacts.__setitem__("answer_assembly_contract", {"decision": "ok"}) or ctx),
    )
    monkeypatch.setattr(
        turn_service,
        "answer_validate_stage",
        lambda ctx, _runtime: (ctx.artifacts.__setitem__("answer_validation_contract", {"passed": True}) or ctx),
    )
    monkeypatch.setattr(
        turn_service,
        "answer_render_stage",
        lambda ctx, _runtime: (ctx.artifacts.__setitem__("answer_render_contract", {"text": "ok"}) or ctx),
    )
    monkeypatch.setattr(turn_service, "answer_commit_stage", lambda ctx, _runtime: ctx)
    ...
    explicit_prior_continuity = ContinuityReadModel(
        committed_turn=CommittedTurnContinuity(
            turn_id="prior-turn",
            commit_stage="answer.commit",
            pending_ingestion_request_id="ingest-from-continuity",
        )
    )

    turn_service.run_canonical_turn_pipeline_service(
        runtime={},
        llm=object(),
        store=object(),
        state=PipelineState(user_input="hello", confidence_decision={}),
        utterance="hello",
        prior_pipeline_state=PipelineState(
            user_input="prior",
            confidence_decision={},
            commit_receipt={"pending_ingestion_request_id": "ingest-from-compat"},
        ),
        prior_continuity=explicit_prior_continuity,
        turn_id="t-2",
        near_tie_delta=0.05,
        chat_history=deque(),
        capability_status="ask_unavailable",
    ...
    assert captured["prior_continuity"] is explicit_prior_continuity
    assert captured["pending_ingestion_request_id"] == "ingest-from-continuity"
