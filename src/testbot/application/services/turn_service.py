...
def run_canonical_turn_pipeline_service(
    *,
    ...
    def _observe_turn(ctx: CanonicalTurnContext) -> CanonicalTurnContext:
        deps.validate_and_log_transition(validate_observe_turn_pre(ctx.state))
    ...
    def _encode_candidates(ctx: CanonicalTurnContext) -> CanonicalTurnContext:
        deps.validate_and_log_transition(validate_encode_candidates_pre(ctx.state))
        rewritten_state = deps.stage_rewrite_query(llm, ctx.state)
        deps.validate_and_log_transition(validate_encode_candidates_post(rewritten_state))
        rewritten_query = rewritten_state.rewritten_query
        deps.append_session_log("query_rewrite_output", {"utterance": utterance, "query": rewritten_query})
    ...
    def _stabilize_pre_route(ctx: CanonicalTurnContext) -> CanonicalTurnContext:
        deps.validate_and_log_transition(validate_stabilize_pre_route_pre(ctx.state))
        ...
        reflection_yaml = deps.generate_reflection_yaml(llm, speaker="user", text=utterance)
        ...
        ctx.state, stabilized = stabilize_pre_route(
            ...
            segment=segment,
            store_doc_fn=deps.store_doc_fn,
        )
    ...
    def _context_resolve(ctx: CanonicalTurnContext) -> CanonicalTurnContext:
        deps.validate_and_log_transition(validate_context_resolve_pre(ctx.state))
        ...
        context_resolution = deps.resolve_context_fn(
    ...
    def _intent_resolve(ctx: CanonicalTurnContext) -> CanonicalTurnContext:
        deps.validate_and_log_transition(validate_intent_resolve_pre(ctx.state))
        stabilized: StabilizedTurnState = ctx.artifacts["stabilized_turn_state"]
        stabilized_utterance = next((fact.value for fact in stabilized.candidate_facts if fact.key == "utterance_raw"), "")
        ...
        classifier_confidence = deps.intent_classifier_confidence(
            utterance=stabilized_utterance or utterance,
            predicted_intent=intent_resolution.classified_intent,
        )
        intent_classifier_metadata: dict[str, object] = {}
        classifier_model = deps.optional_string(runtime.get("intent_classifier_model"))
        classifier_version = deps.optional_string(runtime.get("intent_classifier_version"))
        ...
        guard_forced_memory_retrieval = deps.should_force_memory_retrieval_for_identity_recall(
        ...
        ctx.state = replace(
            ...
            confidence_decision={
                **ctx.state.confidence_decision,
                ...
                "intent_classifier_threshold": deps.intent_classifier_confidence_threshold,
    ...
    def _retrieve_evidence(ctx: CanonicalTurnContext) -> CanonicalTurnContext:
        background_poll = deps.poll_background_source_ingestion(runtime=runtime)
        ...
        if bool(retrieval_requirement.get("requires_retrieval", False)):
            deps.validate_and_log_transition(validate_retrieve_evidence_pre(ctx.state))
            ...
            deps.validate_and_log_transition(validate_retrieve_evidence_post(ctx.state))
            if not docs_and_scores and bool(runtime.get("source_ingest_async_continuation", False)):
                start_result = deps.start_background_source_ingestion(
                    runtime=runtime,
                    store=store,
                )
    ...
    def _policy_decide(ctx: CanonicalTurnContext) -> CanonicalTurnContext:
        ...
        if requires_retrieval:
            deps.validate_and_log_transition(validate_policy_decide_pre(ctx.state))
            stabilized: StabilizedTurnState = ctx.artifacts["stabilized_turn_state"]
            ctx.state, hits = deps.stage_rerank(
                ctx.state,
            ...
            deps.validate_and_log_transition(validate_policy_decide_post(ctx.state))
            ...
            selected_decision = deps.selected_decision_from_confidence(ctx.state.confidence_decision)
            ...
            minimal_confidence = deps.minimal_confidence_decision_for_direct_answer(
    ...
    def _answer_assemble(ctx: CanonicalTurnContext) -> CanonicalTurnContext:
        deps.validate_and_log_transition(validate_answer_assemble_pre(ctx.state))
        ctx.state, answer_routing = deps.resolve_answer_routing_for_stage(
            ctx.state,
            capability_status=capability_status,
            selected_decision=ctx.artifacts["decision_object"],
        )
        assembled = deps.answer_assemble(
            llm,
            ctx.state,
        ...
        offer_type = (
            deps.detect_capability_offer(assembled.final_answer)
    ...
    def _answer_validate(ctx: CanonicalTurnContext) -> CanonicalTurnContext:
        deps.validate_and_log_transition(validate_answer_validate_pre(ctx.state, ctx.artifacts))
        ...
        validated_answer = deps.answer_validate(
            ctx.state,
    ...
    def _answer_render(ctx: CanonicalTurnContext) -> CanonicalTurnContext:
        deps.validate_and_log_transition(validate_answer_render_pre(ctx.state, ctx.artifacts))
